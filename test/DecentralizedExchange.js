...
contract("Decentralized Exchange", (accounts) => {
    ...
    //     console.log("orders[0].filled:", orders[0].filled);
    //     console.log("web3.utils.toWei('5'):", web3.utils.toWei("5"));
    //     assert(orders.length >= 1, "No buy orders found");
    ...
    // it("Should create Market Order & Match.", async () => {
    //     // Deposit 100 FNX tokens to trader1's account
    //     await decentralizedexchange.deposit(web3.utils.toWei("100"), FNX, { from: trader1 });
      
    //     // Create a buy limit order for 10 HLX tokens at a price of 10 tokens per ETH, placed by trader1
    //     await decentralizedexchange.createLimitOrder(HLX, web3.utils.toWei("10"), 10, SIDE.BUY, { from: trader1 });
      
    //     // Deposit 100 HLX tokens to trader2's account
    //     await decentralizedexchange.deposit(web3.utils.toWei("100"), HLX, { from: trader2 });
      
    //     // Create a sell market order for 5 HLX tokens, placed by trader2
    //     await decentralizedexchange.createMarketOrder(HLX, web3.utils.toWei("5"), SIDE.SELL, { from: trader2 });
      
    //     // Check the balances of all four traders after the order matching has taken place
    //     const balances = await Promise.all([
    //       decentralizedexchange.traderBalances(trader1, FNX),
    //       decentralizedexchange.traderBalances(trader1, HLX),
    //       decentralizedexchange.traderBalances(trader2, FNX),
    ...
    //     // Check that the order was successfully matched
    ...
    //     // Check that the balances of all four traders are correct after the order matching has taken place
    //     assert(balances[0].toString() === web3.utils.toWei("50"));
    //     assert(balances[1].toString() === web3.utils.toWei("5"));
    //     assert(balances[2].toString() === web3.utils.toWei("50"));
    //     assert(balances[3].toString() === web3.utils.toWei("95"));
    ...
    it("Should create Market Order & Match.", async () => {
        ...
        const orders = await decentralizedexchange.getOrders(HLX, SIDE.BUY);
        assert.isAtLeast(orders.length, 1, "No buy orders found");
        assert.strictEqual(orders.length, 1);
        assert.strictEqual(orders[0].filled, web3.utils.toWei("5"));
        assert.strictEqual(balances[0].toString(), web3.utils.toWei("50"));
        assert.strictEqual(balances[1].toString(), web3.utils.toWei("5"));
        assert.strictEqual(balances[2].toString(), web3.utils.toWei("50"));
        assert.strictEqual(balances[3].toString(), web3.utils.toWei("95"));
    });
